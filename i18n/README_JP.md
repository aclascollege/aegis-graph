<div align="center">
  <img src="https://avatars.githubusercontent.com/u/195760091?v=4" width="120" height="120" alt="ACLAS Logo" style="border-radius: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.15)">
  
  # ð¡ï¸?Aegis-Graph
  ### ä¸»æ¨©çå­¦è¡ç£æ»ããã³è«çæ¤è¨¼ãã­ãã³ã?  
  [![Documentation](https://img.shields.io/badge/Docs-GitBook-0070f3?style=for-the-badge&logo=gitbook&logoColor=white)](https://atlanta-college-of-liberal-arts.gitbook.io/atlanta-college-of-liberal-arts-and-sciences)
  [![Live Demo](https://img.shields.io/badge/Live-Demo-00dfd8?style=for-the-badge&logo=google-chrome&logoColor=white)](https://aclascollege.github.io/aegis-graph/)
  [![License](https://img.shields.io/badge/License-CC_BY--NC_4.0-lightgrey?style=for-the-badge)](LICENSE)
  
  **ãä¸»æ¨?AI ã¨ã¨ã¼ã¸ã§ã³ãåã¤ã³ããªã¸ã§ã³ã¹ã§æè²ã®æªæ¥ãå®ããã?*
</div>

---

### ð ã°ã­ã¼ãã«ã¢ã¯ã»ã·ããªãã?(å¤è¨èª?

| ð å°å | è¨èªãããªãã¯ã?|
| :--- | :--- |
| **ååã¢ã¡ãªã« / EMEA** | [🇺🇸 English](../README.md) â?[🇫🇷 FranÃ§ais](README_FR.md) â?[🇪🇸 Español](README_ES.md) â?[🇩�� Deutsch](README_DE.md) â?[🇵🇹 PortuguÃªs](README_PT.md) |
| **ã¢ã¸ã¢å¤ªå¹³æ´** | [🇭🇰 ç¹é«ä¸­æ](README_ZH.md) â?[🇯🇵 æ¥æ¬èª](README_JP.md) â?[🇰🇷 íêµ­ì´](README_KR.md) |
| **ä¸­æ±** | [🇸🇦 Ø§ÙØ¹Ø±Ø¨ÙØ© (RTL)](README_AR.md) |

---

## ðï¸?æ©é¢ã®èæ?
**Aegis-Graph** ã?[**ã¢ãã©ã³ã¿ã»ãªãã©ã«ã¢ã¼ãã»ãµã¤ã¨ã³ã¹å¤§å­?(ACLAS College)**](https://aclas.college/) ã®æè¡çãªæè¦ãã­ã¸ã§ã¯ãã§ããAI ã«ãã£ã¦çæãããé«ç²¾åº¦ã®å­¦è¡ä¸æ­£ã«å¯¾æããããã®ä¸»æ¨©ãã¼ãã¨ãã¦è¨­è¨ããã¦ãã¾ãã?
---

## ð æè¡ã®ä¸­æ ¸: Agentic GraphRAG

### ð¤ ã¨ã¼ã¸ã§ã³ãã»ã¹ã¦ã©ã¼ã ã®åè§?- **è¦è¦ãã©ã¬ã³ã¸ãã¯ã»ã¨ã¼ã¸ã§ã³ã**: ãã¤ãºãã¿ã¼ã³ããã©ã³ãã®ç°å¸¸ãåæããåæãã­ã¥ã¡ã³ããæ¤åºãã¾ãã?- **ã°ã©ãããã²ã¼ã¿ã¼ã»ã¨ã¼ã¸ã§ã³ã**: OpenAlex (2.5åä»¶ä»¥ä¸ã®è¨é? ã?ROR ãè·¨ãã§ãã«ããããã¯ã¨ãªãå®è¡ãã¾ãã?- **ã­ã¸ãã¯ç£æ»ã¨ã¼ã¸ã§ã³ã**: åæ¥­ã®ã¿ã¤ã ã©ã¤ã³ãåä½ã®è«çæ§ãæ¤è¨¼ãã¾ãã?
---

## ð ï¸?ã¯ã¤ãã¯ã¹ã¿ã¼ã

```bash
# ãªãã¸ããªã®ã¯ã­ã¼ã³
git clone https://github.com/aclascollege/aegis-graph.git
cd aegis-graph

# ã¤ã³ã¹ãã¼ã«
pip install -r requirements.txt

# ç£æ»ã®å®è¡?python main_pipeline.py --input examples/sample_transcript.pdf
```

---

<div align="center">
  <p>© 2026 Atlanta College of Liberal Arts and Sciences (ACLAS College). All rights reserved.</p>
</div>
